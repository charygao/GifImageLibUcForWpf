...
namespace GifImageLib
{
    internal class GifAnimation : Viewbox
    {
        ...
        public void CreateGifAnimation(MemoryStream memoryStream)
        {
            ...
            foreach (var t in _frameList)
                Console.WriteLine($@"{t.DisposalMethod} {t.ThisWidth} {t.DelayTime}");

            if (_frameList.Count > 1)
            {
                if (_numberOfLoops == -1) _numberOfLoops = 1;
        ...
        public void NextFrame(object sender, EventArgs e)
        {
            ...
            if (_frameList[_frameCounter].DisposalMethod == 2) _frameList[_frameCounter].Visibility = Visibility.Hidden;
            if (_frameList[_frameCounter].DisposalMethod >= 3) _frameList[_frameCounter].Visibility = Visibility.Hidden;
            _frameCounter++;
            ...
            {
                if (_numberOfLoops != 0) _currentLoop++;
                if (_currentLoop < _numberOfLoops || _numberOfLoops == 0)
                {
                    ...
                    _frameTimer.Interval =
                        new TimeSpan(0, 0, 0, 0, _frameList[_frameCounter].DelayTime * 10);
                    _frameTimer.Start();
                }
            }
        }
        ...
        private int ParseGraphicBlock(byte[] gifData, int offset)
        {
            ...
            _currentParseGifFrame.ThisWidth  = BitConverter.ToUInt16(gifData, offset + 5);
            _currentParseGifFrame.ThisHeight = BitConverter.ToUInt16(gifData, offset + 7);
            if (_currentParseGifFrame.ThisWidth  > _logicalWidth) _logicalWidth   = _currentParseGifFrame.ThisWidth;
            if (_currentParseGifFrame.ThisHeight > _logicalHeight) _logicalHeight = _currentParseGifFrame.ThisHeight;
        ...
        private int ParseExtensionBlock(byte[] gifData, int offset)
        {
            ...
            if (gifData[offset + 1] == 0xFF && length > 10)
            {
                ...
                if (netscape == "NETSCAPE")
                {
                    ...
                    if (_numberOfLoops > 0) _numberOfLoops++;
                }
            }
        ...
        private int ParseHeader(byte[] gifData, int offset)
        {
            ...
            if (str != "GIF") throw new Exception("Not a proper GIF file: missing GIF header");
            return 6;
        }
